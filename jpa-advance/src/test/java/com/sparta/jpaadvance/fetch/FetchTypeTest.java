...
public class FetchTypeTest {
    ...
    void test1() {
        Food food = foodRepository.findById(2L).orElse(null);
    ...
    void test2() {
        // User는 @OneTo"Many"이기 때문에 default가 지연 로딩이 걸려있음
        // user.getName()은 left join 없이 select만 호출됨
        User user = userRepository.findByName("Robbie");
        System.out.println("user.getName() = " + user.getName());

        // Food 테이블이 연결되어 SQL 쿼리 호출됨
        // foodList를 가지고 있는 User 클래스의 PK 정보를 사용하여 Food 테이블에 조회함
