...
@WebMvcTest(
        controllers = {UserController.class, ProductController.class},  // 테스트할 controller 설정
...
class UserProductMvcTest {
    ...
    @MockBean // 주입을 해주기 위해 가짜 bean을 만드는 것
    ...
    public void setup() {
        mvc = MockMvcBuilders.webAppContextSetup(context)
                .apply(springSecurity(new MockSpringSecurityFilter())) // 가짜 filter가 가짜 security로써 사용됨
                .build();
    }

    private void mockUserSetup() { // Controller에서 authentication 사용하여 사용자를 뽑아내는데, 그 부분을 동작시키기 위해 세팅하는 것
        // Mock 테스트 유져 생성
    ...
    void test3() throws Exception {
        ...
        String postInfo = objectMapper.writeValueAsString(requestDto); // 객체를 Json 형식의 String으로 내보내줌
